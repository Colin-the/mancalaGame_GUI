...
public class KalahRulesTest {
    ...
    @Test
        public void addStonesToStore() throws InvalidMoveException {
        
            //player one move stones starting at pit 6
            int num = rules.moveStones(3,1);
            assertEquals(1,player1.getStoreCount());  //number in players store
            //pit 6 has zero stones
            assertEquals(0,data.getNumStones(3));
            //pit 8 has 5 stones
            assertEquals(5,data.getNumStones(4));
            num = rules.moveStones(4,1);
            assertEquals(2,player1.getStoreCount());  //number in players store
            assertEquals(5,data.getNumStones(8));
    }
  ...
  @Test
    public void myTestMoveStonesWithCapture() throws InvalidMoveException {
        //add stones to pit 6 to total 8 stones
        data.addStones(6,4);
        //remove all stones from pit 1
        data.removeStones(1);
       ...
       assertEquals(6, player1.getStoreCount());    
